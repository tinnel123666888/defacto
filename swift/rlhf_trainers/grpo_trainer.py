...
class GRPOTrainer(RolloutTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
        ...
        self._timing_metrics = defaultdict(list)  # {metric_name: [values]}
        self._timing_step_count = 0
        self._timing_log_every = 10  # Log timing stats every N steps
        
        # ========== TIMING FILE WRITER ==========
        timing_log_file = os.path.join(self.args.output_dir, 'training_timing.jsonl')
        self._timing_writer = JsonlWriter(timing_log_file)
        self._step_timing_data = {}  # Cache for current step's timing data

    def _get_data_collator(self, args, template):
        return identity_data_collator

    @contextmanager
    def step_timer(self, phase_name: str):
        """Context manager for timing specific phases of training"""
        import torch
        ...
        try:
            ...
            self._timing_metrics[phase_name].append(duration)
            # Also track in step-level data for immediate file logging
            if phase_name not in self._step_timing_data:
                self._step_timing_data[phase_name] = []
            self._step_timing_data[phase_name].append(duration)
    
    def _log_timing_stats(self, step_num: int):
        """Log accumulated timing statistics and save to file"""
        if not self.accelerator.is_main_process:
            return
        
        msg_parts = [f'\n[Step {step_num}] Detailed timing statistics:']
        timing_data_for_file = {'step': step_num, 'timestamp': time.time()}
        
        for metric_name in sorted(self._timing_metrics.keys()):
            values = self._timing_metrics[metric_name]
            if values:
                avg = sum(values) / len(values)
                min_val = min(values)
                max_val = max(values)
                total = sum(values)
                msg_parts.append(f'  {metric_name}: avg={avg:.2f}ms, min={min_val:.2f}ms, max={max_val:.2f}ms, total={total:.2f}ms, count={len(values)}')
                # Save to file
                timing_data_for_file[f'{metric_name}_avg_ms'] = avg
                timing_data_for_file[f'{metric_name}_min_ms'] = min_val
                timing_data_for_file[f'{metric_name}_max_ms'] = max_val
                timing_data_for_file[f'{metric_name}_total_ms'] = total
                timing_data_for_file[f'{metric_name}_count'] = len(values)
        
        logger.warning('\n'.join(msg_parts))
        
        # Write to jsonl file
        try:
            self._timing_writer.append(timing_data_for_file)
        except Exception as e:
            logger.warning(f'Failed to write timing data to file: {e}')
        
        # Log to wandb if available
        if is_wandb_available() and 'wandb' in self.args.report_to and wandb.run is not None:
            timing_log = {f'timing/{k}_avg_ms': sum(v)/len(v) for k, v in self._timing_metrics.items() if v}
            timing_log[f'timing/step'] = step_num
    ...
    def _prepare_inputs(self, generation_batch: Dict[str, Union[torch.Tensor,
        ...
        with self.step_timer('phase_prepare_inputs_total'):
            ...
            if mode == 'train':
                ...
                if self._step % generate_every == 0 or self._buffered_inputs is None:
                    with self.step_timer('phase_generate_and_score_cycle'):
                        generation_batch = self._generate_and_score_completions(generation_batch)
                    self._buffered_inputs = generation_batch  # < this is the change
                else:
                    # Reusing buffered rollout outputs can still be a meaningful slice/index overhead to track.
                    with self.step_timer('phase_reuse_buffer'):
                        pass
    ...
    def _generate_and_score_completions(self, inputs: DataType) -> DataType:
        ...
        if self.template.truncation_strategy == 'raise':
            with self.step_timer('phase_resample_encode_failed'):
                inputs = self.resample_encode_failed_inputs(inputs)

        with self.step_timer('phase_generate_completions'):
            inputs = self._generate_completions(inputs)
        with self.step_timer('phase_score_completions'):
        ...
        with self.step_timer('phase_split_mini_batches'):
        ...
        with self.step_timer('phase_attach_advantages'):
        ...
        with self.step_timer('phase_log_metrics'), profiling_context(self, 'log_metrics'):
    ...
    def _compute_rewards_per_func(self, inputs: DataType) -> torch.Tensor:
        ...
        for i, (reward_func, reward_model_plugin, reward_func_name) in enumerate(
            ...
            with self.step_timer(f'reward_{reward_func_name}'), profiling_context(self, reward_func_name), self._disable_sp_context(template):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        with self.step_timer('compute_loss_total'):
    ...
    def _compute_loss_and_metrics(self, model, inputs):
        ...
        # ===== PHASE 1: Forward pass computation =====
        with self.step_timer('phase_model_forward'):
    ...
    def training_step(self, model: nn.Module, inputs: DataType, num_items_in_batch=None) -> torch.Tensor:
        ...
        if self.args.async_generate:
            ...
            with self.step_timer('async_wait'):
                while not self.is_async_generate_eval_rollout_done():
                    time.sleep(0.01)
        
        with self.step_timer('full_training_step'):
            output = super().training_step(model, inputs, num_items_in_batch)
        
        step_end = time.perf_counter()
        step_duration = (step_end - step_start) * 1000  # Convert to ms
        
        # Save timing data for every training step
        if self.accelerator.is_main_process:
            timing_data = {
                'global_step': self.state.global_step,
                'step': self.state.global_step,
                'timestamp': time.time(),
                'total_step_time_ms': step_duration,
            }
            
            # Add phase-specific timings
            for phase_name, times in self._step_timing_data.items():
                if times:
                    # Sum all occurrences of the same phase within one step.
                    timing_data[f'{phase_name}_ms'] = sum(times)
                    timing_data[f'{phase_name}_count'] = len(times)

            # Compute unclassified time inside the training step.
            classified_ms = 0.0
            for key, val in timing_data.items():
                if not key.endswith('_ms'):
                    continue
                if key in {'total_step_time_ms', 'phase_other_ms'}:
                    continue
                if isinstance(val, (int, float)):
                    classified_ms += float(val)
            timing_data['phase_other_ms'] = max(0.0, step_duration - classified_ms)
            
            try:
                self._timing_writer.append(timing_data)
            except Exception as e:
                logger.debug(f'Failed to write step timing data: {e}')
        
        # Clear step timing data
        self._step_timing_data = {}
        
        # Record step count and log summary periodically
        self._timing_step_count += 1
        if self._timing_step_count % self._timing_log_every == 0:
            self._log_timing_stats(self.state.global_step)
    ...
    def offload_context(self):
        ...
        try:
            ...
            logger.warning(f'Failed to offload during context entry: {offload_exc}. Continuing without offload.')

        origin_exc: Optional[Exception] = None

        def _is_cuda_illegal_access(exc: Exception) -> bool:
            msg = str(exc).lower()
            return 'illegal memory access' in msg or 'cudaerrorillegaladdress' in msg

        def _run_reload(op_name: str, op):
            try:
                op()
            except Exception as reload_exc:
                if origin_exc is not None:
                    logger.error('Skip secondary exception while %s during failure cleanup: %s', op_name, reload_exc)
                    return
                raise
        ...
        try:
            ...
            skip_reload = origin_exc is not None and _is_cuda_illegal_access(origin_exc)
            if skip_reload:
                logger.error(
                    'Detected CUDA illegal memory access. Skip model/optimizer reload in offload_context to avoid '
                    'cascading CUDA errors. The process should be restarted.')
            # reload (load back) model when exiting context
            if not skip_reload:
                if self.args.offload_model:
                    _run_reload('load_model(main)', lambda: self.load_model(self.accelerator.unwrap_model(self.model)))
                    if self.ref_model:
                        _run_reload('load_model(ref)', lambda: self.load_model(self.ref_model))
                if getattr(self, 'optimizer', None) and self.args.offload_optimizer:
                    _run_reload('load_optimizer', self.load_optimizer)
