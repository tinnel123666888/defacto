...
def _is_cuda_init_error(exc: Exception) -> bool:
    ...
    return ('cuda' in msg or 'acceleratorerror' in name or 'cuda' in name)
...
class RolloutTrainerMixin(RLHFTrainerMixin):
    ...
    def _prepare_vllm_engine(self):
        ...
        # Multi-node sync before vLLM init to prevent timing/state inconsistencies
        if torch.distributed.is_initialized():
            logger.info(f'[Rank {torch.distributed.get_rank()}] Waiting for all nodes before vLLM init...')
            torch.distributed.barrier()
            logger.info(f'[Rank {torch.distributed.get_rank()}] All nodes ready, proceeding with vLLM init')
        ...
        with Swift.grpo_context(model, self.template.processor):
            ...
            try:
                ...
                # Copy to avoid mutating user-provided kwargs across retries.
                base_engine_kwargs = deepcopy(self.args.vllm_engine_kwargs or {})
                load_format = base_engine_kwargs.pop('load_format', 'dummy')

                engine = None
                max_retries = 4 if self.is_multimodal else 3
                base_gpu_memory_util = self.vllm_gpu_memory_utilization

                for retry_count in range(max_retries):
                    retry_gpu_memory_util = max(0.2, base_gpu_memory_util * (0.8**retry_count))
                    retry_max_num_seqs = max(1, max_num_seqs // (2**retry_count)) if self.is_multimodal else max_num_seqs
                    retry_enable_prefix_caching = args.vllm_enable_prefix_caching and retry_count == 0
                    retry_enforce_eager = args.vllm_enforce_eager or retry_count > 0
                    retry_mm_processor_cache_gb = 0 if self.is_multimodal and retry_count > 0 else args.vllm_mm_processor_cache_gb

                    try:
                        # Use consistent seed across all nodes for multimodal profile_run.
                        # Different seeds can cause shape mismatches in dummy inputs during
                        # vLLM's KV cache profiling, triggering illegal memory access.
                        vllm_seed = 42 + retry_count  # Same for all nodes, vary by retry only

                        # Keep host/cuda RNG state aligned across ranks before engine creation.
                        random.seed(vllm_seed)
                        torch.manual_seed(vllm_seed)
                        if torch.cuda.is_available():
                            torch.cuda.manual_seed_all(vllm_seed)
                        
                        logger.info(
                            f'[Rank {self.accelerator.process_index}] vLLM init attempt {retry_count+1}/{max_retries}: '
                            f'gpu_mem={retry_gpu_memory_util:.2f}, max_seqs={retry_max_num_seqs}, '
                            f'eager={retry_enforce_eager}, seed={vllm_seed}'
                        )
                        ...
                        engine = GRPOVllmEngine(
                            ...
                            gpu_memory_utilization=retry_gpu_memory_util,
                            enable_prefix_caching=retry_enable_prefix_caching,
                            max_num_seqs=retry_max_num_seqs,
                            enforce_eager=retry_enforce_eager,
                            ...
                            mm_processor_cache_gb=retry_mm_processor_cache_gb,
                            template=vllm_template,
                            distributed_executor_backend='external_launcher',
                            engine_kwargs=deepcopy(base_engine_kwargs),
                            logprobs_mode=logprobs_mode,
                            **lora_kwargs,
                        )
                        if torch.distributed.is_initialized():
                            torch.distributed.barrier()
                        logger.info(f'[Rank {self.accelerator.process_index}] vLLM engine initialized successfully')
                        break
                    except Exception as e:
                        if _is_cuda_init_error(e) and retry_count < max_retries - 1:
                            logger.warning(
                                'vLLM engine init failed (retry %s/%s): gpu_memory_utilization=%.2f, '
                                'max_num_seqs=%s, enforce_eager=%s, mm_processor_cache_gb=%s. Error: %s',
                                retry_count + 1,
                                max_retries,
                                retry_gpu_memory_util,
                                retry_max_num_seqs,
                                retry_enforce_eager,
                                retry_mm_processor_cache_gb,
                                str(e)[:300],
                            )
                            aggressive_empty_cache()
                            # Illegal-address failures can poison the CUDA context; next retry is best-effort.
                            if _is_cuda_illegal_access(e):
                                time.sleep(0.2)
                            continue
                        raise

                if engine is None:
                    raise RuntimeError('Failed to initialize vLLM engine after retries')
            finally:
    ...
    def offload_model(self, model):
        try:
            ...
            # CUDA may already be in corrupted state (e.g., invalid argument error)
            # Log warning and continue - vLLM initialization may still proceed
            logger.warning(f'Failed to offload model to CPU: {e}. Continuing without offload.')
            try:
                torch.cuda.empty_cache()
            except:
                pass
