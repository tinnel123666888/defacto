...
def get_model_info_meta(
    ...
    # Check for invalid model IDs before attempting to download
    if model_meta is None and model_type is None:
        # Model not found in registry, don't attempt download
        model_dir_name = model_id_or_path.split('/')[-1].split(':')[0]
        if model_dir_name.lower() in ('unknown', 'none', 'null', ''):
            raise ValueError(f'Invalid model_id_or_path: "{model_id_or_path}". Please provide a valid model ID.')
